...
- MFA sign-in, backup-factor lifecycle, one successful masked password-recovery path, and wrong-code recovery rejection passed. The rejected recovery left the existing password valid and the primary authenticator completed the subsequent AAL2 sign-in. Primary-specific and backup-specific successful recovery, email-only bypass rejection, simulated assurance/factor lookup failure, privacy-safe audit-content review, and the all-factors-lost support process remain required before this task can be marked Verified.
