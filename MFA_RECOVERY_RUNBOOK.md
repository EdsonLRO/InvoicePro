...
- Authenticated generation, replacement, one-time recovery, factor/session cleanup, recovery-state lock, audit metadata, two-account isolation, and rollback-only production throttling have passed with privacy-safe evidence. The throttling probe returned four invalid attempts followed by a 15-minute lock on attempt five, then rolled back and left no failed-attempt or lock state. Notification-email delivery, final legal disposition, and real-device browser acceptance remain required before the frontend is published.
...
4. **In progress:** unauthorized, wrong-origin, rolled-back AAL/RLS, authenticated generation, replacement, one-time-use, forced re-enrolment, audit-minimisation, two-account, and rollback-only live-throttling probes passed. Notification delivery and real-device acceptance remain.
