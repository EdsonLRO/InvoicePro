...
| Refund/dispute/chargeback handling | In Progress | Technical sandbox handling is verified, and the authenticated full live-refund path now passes provider request, signed webhook reconciliation, single mutation, audit and balance-restoration checks. The internal procedure is recorded in `PAYMENT_OPERATIONS_RUNBOOK.md`. Chargeback/support operations, customer-facing policy and legal review remain open. |
