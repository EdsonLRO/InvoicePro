...
| Commercial Billing/Connect foundations | In Progress | The disabled foundations and 2026-07-24 deployment baseline pass. On 2026-07-25 the approved Stripe sandbox Prices, signed Billing destination, Portal and private test settings supported one protected synthetic monthly subscription. Signed activation, Portal return, period-end cancellation, duplicate/stale handling and rollback-only renewal/failure/grace/read-only/recovery probes pass. PR #103 merged the unapplied entitlement migration and server guards. Only the two approved Connect functions were deployed, the Owner privately configured the Connect secret names, and the sandbox gates are enabled with live mode off. PRs #104-#107 corrected the Accounts v2 payout request, UK country, indexed retrieval and trusted Account Link flow. Only `manage-stripe-connect` advanced from version 18 to 19 for PR #107; JWT verification remained enabled and every other function remained unchanged. Stripe-hosted sandbox onboarding now opens and Account Links v2 returned HTTP 200. Owner-private completion, the second isolated account and connected payment/refund and failure/replay acceptance remain. Existing live Owner functions were not redeployed. Evidence: `STRIPE_BILLING_SANDBOX_ACCEPTANCE_EVIDENCE_2026-07-25.md`. |
