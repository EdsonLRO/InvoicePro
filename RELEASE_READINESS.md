...
| Audit events | In Progress | Provider and selected app actions covered; failed-email and overdue-reminder failure evidence is minimised. Authenticated `log-app-event` v5 adds manual payment recording and document-status changes with category/amount/status metadata only. This is not a full SIEM or compliance system. |
| Account-holder data export | Implemented | Authenticated RLS-scoped JSON export, stable pagination, limited Auth metadata, whole-export failure, session-change abort, trusted-device warning, local browser creation, and minimal audit metadata are implemented. Focused success/failure tests pass. Dedicated test-account desktop/mobile acceptance and deployed audit-event readback remain. This is not a complete third-party rights-request workflow. |
