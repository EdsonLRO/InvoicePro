...
| Edge Function dependency pinning | Verified | All nine function sources pin `@supabase/supabase-js` to exact version `2.110.1`; unused floating aliases were removed, the repository harness rejects floating Supabase specifications, and every function passed `deno check`. Production functions were not redeployed for this source-only reproducibility control. |
...
| Audit events | In Progress | Provider and selected app actions covered; failed-email and overdue-reminder failure evidence is minimised. Authenticated `log-app-event` v6 adds manual payment recording, document-status changes, and minimal account-export success metadata. This is not a full SIEM or compliance system. |
| Account-holder data export | Implemented | Authenticated RLS-scoped JSON export, stable pagination, limited Auth metadata, whole-export failure, session-change abort, trusted-device warning, local browser creation, and minimal audit metadata are implemented. Focused success/failure tests, PR #29, Pages publication, v6 allowlist source readback, and clean immediate logs pass. Dedicated test-account desktop/mobile acceptance and one successful production event row remain. This is not a complete third-party rights-request workflow. |
| Privacy/legal groundwork | In Progress | Internal data flow, provisional role matrix, working ROPA, vendor/transfer and retention registers, case templates, DPIA screening, rights/incident procedures, claims controls, a fictional tabletop, and an account-export review are recorded. Public notices, final decisions, restricted case tooling, correction/deletion/provider-assistance operations, verified live procedures and professional review remain unfinished. |
