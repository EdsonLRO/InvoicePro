...
| All-factors-lost account recovery | In Progress | Backend deployment is complete: migrations through `20260716161054`, server-only pepper, and JWT-protected `mfa-recovery` version 1 are live. Boundary, privilege, AAL, rolled-back RLS, authenticated generation/replacement/recovery, audit-minimisation, two-account, and rollback-only live-throttling probes pass. Notification delivery, final Legal Agent condition closure, and real-device acceptance remain before frontend publication or paid/public onboarding. |
