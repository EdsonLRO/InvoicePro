...
| Privacy/legal groundwork | In Progress | Internal data flow, provisional role matrix, working ROPA, vendor/transfer and retention registers, case templates, DPO/DPIA screening, rights/incident procedures, claims controls, a fictional tabletop, and an account-export review are recorded. The Owner selected an Owner-operated model and Business Standard as the case-system candidate, but the subscription is pending; Vault, a backup operator, paid Workspace support and a cyber retainer remain optional. Public notices, final decisions, provider purchase/configuration, correction/deletion/provider-assistance operations, synthetic acceptance and the focused one-off legal review remain unfinished. |
