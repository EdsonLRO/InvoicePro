...
| Enforced security gate on `main` | Implemented | Active GitHub ruleset `18994100` targets default `main`, has no bypass actors, blocks deletion/force-push, requires pull requests with zero approvals, and requires `verify` against the latest branch state. Focused pull-request enforcement proof is pending. |
