...
| Commercial Billing/Connect foundations | In Progress | The disabled foundations and 2026-07-24 deployment baseline pass. On 2026-07-25 the approved Stripe sandbox Prices, signed Billing destination, Portal and private test settings supported one protected synthetic monthly subscription. Signed activation, Portal return, period-end cancellation, duplicate/stale handling and rollback-only renewal/failure/grace/read-only/recovery probes pass. PR #103 merged the unapplied entitlement migration and server guards. Only the two approved Connect functions were deployed, the Owner privately configured the Connect secret names, and the sandbox gates are enabled with live mode off. PRs #104-#107 corrected the Accounts v2 onboarding path, and the Owner privately completed the first synthetic account's hosted onboarding; card payments and payouts now report ready. The first fictional GBP 1 Checkout request returned a controlled HTTP 502 before Checkout creation because the shared account refresh still used obsolete unindexed Accounts v2 includes. A focused source-and-test correction is under review. No connected payment or refund occurred, and existing live Owner functions were not redeployed. Evidence: `STRIPE_BILLING_SANDBOX_ACCEPTANCE_EVIDENCE_2026-07-25.md`. |
...
| Supabase Auth provider policy | In Progress | Email confirmation, MFA, refresh rotation, leaked-password protection, the 12-character minimum, session timeouts and custom Auth SMTP remain active. Release candidate build `2026.07.25.1` adds only `https://app.tallyo.co.uk` to the MFA recovery origin allowlist while retaining GitHub Pages and both local development origins; it is not deployed. Production Turnstile remains active for the existing reviewed origins, but `SEC-AUTH-006` requires its server secret to be rotated and the final hostname added before public release. The one-hour JWT lifetime and multi-device sessions remain unchanged. |
