...
Tallyo's current invoicing-app scope is feature-complete and regression-verified, and its controlled live Stripe invoice-payment path is activated and acceptance-tested. Build `2026.07.19.1` is published after exact Owner approval, merged PR #74 as `51b1397`, successful required/post-merge checks, and successful Pages/readback verification. The app is not approved for unrestricted live-customer onboarding because the separate legal, privacy and operational gates remain open.

## Release Gates

| Gate | Status | Evidence / next action |
|---|---|---|
| Core invoicing workflows | Verified | The final 2026-07-16 regression remains current. `FUNC-READY-002` additionally completed a clean fictional customer/item, invoice, quote conversion, duplicate, status, standalone credit, PDF/XLSX and recurring create/pause/resume/edit journey without sending email, entering Checkout, moving money or deleting data. Public build `2026.07.19.1` retains that validation and the verified responsive navigation and centred document-type badges, removes the customer-facing status panel, and adds clearer account and workflow wording; all focused harnesses, PR/post-merge checks and Pages/readback verification pass. |
