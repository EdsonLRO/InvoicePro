...
| Commercial Billing/Connect foundations | In Progress | The disabled foundations and 2026-07-24 deployment baseline pass. On 2026-07-25 the approved Stripe sandbox Prices, signed Billing destination, Portal and private test settings supported one protected synthetic monthly subscription. Signed activation, Portal return, period-end cancellation, duplicate/stale handling and rollback-only renewal/failure/grace/read-only/recovery probes pass. PR #103 merged the unapplied entitlement migration and server guards. Only the two approved Connect functions were deployed, the Owner privately configured the Connect secret names, and the sandbox gates are enabled with live mode off. PRs #104 and #105 corrected the obsolete payout request and supplied the approved UK country; only `manage-stripe-connect` advanced to version 17. Stripe created the sandbox connected account successfully, but the follow-up account retrieval rejected obsolete unindexed array query syntax before hosted onboarding. A focused indexed-query correction is under review. No Connect payment or refund occurred, and existing live Owner functions were not redeployed. Evidence: `STRIPE_BILLING_SANDBOX_ACCEPTANCE_EVIDENCE_2026-07-25.md`. |
