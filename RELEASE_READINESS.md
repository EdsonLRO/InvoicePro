...
| Backups and restore | In Progress | Pro daily physical backups were verified through 2026-07-14 with WAL-G enabled and PITR disabled; an Owner-approved timed non-production restore test remains. |
...
| Privacy/legal groundwork | In Progress | Internal data flow, provisional role matrix, vendor/transfer register, retention decisions, rights procedure, incident procedure, and claims controls are recorded in `LEGAL_PRIVACY_READINESS.md`. Public notices, final decisions, operations and professional review remain unfinished. |
