...
| All-factors-lost account recovery | In Progress | A server-managed one-time recovery-code design is implemented and locally verified on `codex/mfa-recovery-codes`. It uses AAL2-only generation, HMAC-only storage, attempt locking, global factor/session cleanup, database-enforced recovery lock, forced TOTP re-enrolment, notices, and minimal audit evidence. Production still uses the approved deny-by-default response. Migration, secret, Edge deployment, live security probes, browser acceptance, and Legal Agent condition closure remain required before paid/public onboarding. |
...
| Edge Function dependency pinning | Verified | All ten function sources, including the undeployed `mfa-recovery` candidate, pin `@supabase/supabase-js` to exact version `2.110.1`; the repository harness rejects floating Supabase specifications and frozen checks pass. The nine production functions were not redeployed for this source-only reproducibility control. |
