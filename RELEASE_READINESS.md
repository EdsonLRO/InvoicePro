...
| Commercial Billing/Connect foundations | In Progress | The disabled foundations and 2026-07-24 deployment baseline pass. On 2026-07-25 the approved Stripe sandbox Prices, signed Billing destination, Portal and private test settings supported one protected synthetic monthly subscription. Signed activation, Portal return, period-end cancellation, duplicate/stale handling and rollback-only renewal/failure/grace/read-only/recovery probes pass. PR #103 merged the unapplied entitlement migration and server guards. Only the two approved Connect functions were deployed, the Owner privately configured the Connect secret names, and the sandbox gates are enabled with live mode off. PR #104 removed the obsolete Accounts v2 payout request and only `manage-stripe-connect` was redeployed. A second onboarding request created no account because Stripe required the UK country before applying Merchant; a focused country-only correction is under review and leaves legal entity type to hosted onboarding. Existing live Owner functions were not redeployed. Evidence: `STRIPE_BILLING_SANDBOX_ACCEPTANCE_EVIDENCE_2026-07-25.md`. |
