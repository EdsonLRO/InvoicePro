...
- [Current pricing](https://supabase.com/pricing) and [logging documentation](https://supabase.com/docs/guides/telemetry/logs) state that the Pro plan exposes seven days of API/database logs and seven days of Auth audit logs. This provider window does not set Tallyo's separate `audit_events` retention.
...
- [Resend's webhook-storage guidance](https://resend.com/docs/dashboard/webhooks/how-to-store-webhooks-data) states that email data is retained for 30 days across standard plans, with flexible Enterprise retention. This is provider evidence, not a reason for Tallyo to create a longer message-body store.
