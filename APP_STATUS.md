...
- PR #117 recorded the completed live provider configuration. Under separate exact approvals, one controlled live subscription completed and one isolated live connected business completed Stripe-hosted onboarding and identity verification; no private identity, banking or payment details were inspected or stored by Codex. PR #118 merged the stale pre-session Checkout-claim repair and migration `20260726172105` was the only pending migration applied. The next protected Connect retry created and persisted an open provider Session but returned HTTP 502 before exposing its URL, so no connected-account payment or refund occurred.
...
- Subscription Checkout is configured server-side for live Stripe mode, and one controlled live acceptance subscription completed. It remains unavailable to visitors because the app and website publication gates are off and the custom app domain remains behind Access.
...
- Independent-business card payments have an applied schema, accepted multi-account sandbox path, enabled server-side live configuration and one completed live onboarding. Live direct-charge Checkout and refund acceptance remain incomplete, and the feature is publicly unavailable.
...
- Complete the remaining controlled live Stripe Connect Checkout/payment/refund acceptance and Billing lifecycle readback only through separate exact payment approvals.
