...
- The controlled redesign is public as build `2026.09.19.2` from Cloudflare deployment `db056e05-2e12-46ec-b076-45215c707831` and merge `c918bfb`. The Owner approved focused follow-up build `2026.09.19.3`: phone previews fit the complete printable invoice without horizontal scrolling, both page edges remain visible, and the visible line-item heading is rounded correctly on phone and desktop. It retains the Owner Console, canonical recovery-link correction, complimentary access, account-data export, branded notices and first-sign-in subscription guidance. The Supabase Auth Site URL remains `https://app.tallyo.co.uk/`; no backend, Auth, provider or website change belongs to this frontend follow-up.
