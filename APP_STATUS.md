...
- MFA browser acceptance is complete for fail-closed routing, primary/backup factor lifecycle, primary-specific and backup-specific recovery, wrong-code rejection, and email-only bypass rejection. The all-factors-lost backend is deployed and has passed boundary, privilege, AAL, and rolled-back RLS probes; authenticated lifecycle and browser/device acceptance remain a paid/public-launch condition before publishing its frontend.
