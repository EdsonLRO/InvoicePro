...
- Source candidate `2026.07.28.3` on `codex/ga4-consent-controls` adds disabled-by-default GA4 consent controls, an exact eight-event allowlist and the reviewed Privacy/Cookie Notice updates. The tag remains absent before affirmative consent and after rejection, and production activation still requires the exact build gate plus separate Owner deployment approval. This candidate is not committed, deployed or active.
