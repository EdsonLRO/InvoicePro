...
- Resolve the recorded Supabase session, SMTP/rate-limit, and abuse-control decisions in `DEFERRED_MANUAL_CONFIGURATION.md`. Leaked-password protection and a verified 12-character provider minimum are enabled. Graceful unexpected-session expiry handling is implemented and harness-tested; the recommended 7-day timebox and 24-hour inactivity timeout remain Owner-gated until deployment acceptance.
