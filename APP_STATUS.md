...
- Supabase does not provide native recovery codes. Tallyo supports a second authenticator and prevents email-only MFA bypass. AUTH-001 acceptance is Verified. AUTH-002's backend controls are deployed with HMAC-only storage and forced re-enrolment; live throttling passed with rollback-only production evidence, while production keeps the interim deny-by-default user process until notification delivery, final legal disposition, and real-device acceptance pass.
