...
- Privacy-safe production reconciliation found eight accounts with business data and only two current full entitlements. Applying the merged entitlement policy without a rollout gate would therefore make six existing accounts read-only. The focused live-Billing release candidate keeps enforcement behind one private database-owner-controlled switch that defaults off, adds explicit mutually exclusive test/live provider gates and permits only matching `cs_test_` or `cs_live_` Checkout claims. Disposable PostgreSQL 17 migration, privilege, RLS and live-session probes pass. Nothing is applied, deployed, configured or publicly enabled by this candidate.
...
- Review and merge the focused live-Billing candidate, then apply its two pending migrations and deploy the exact Billing/entitlement guard function set only through a separate exact approval. Subscription enforcement remains off during controlled live acceptance; enabling the private gate is a later reversible production action after account impact is accepted.
