...
- PR #103 merged the unapplied entitlement-enforcement migration and server guards. Only `manage-stripe-connect` and `create-connect-checkout` were then deployed under exact approval; the entitlement migration and existing live functions remain untouched. The Owner privately configured the Connect key and webhook secret, the four sandbox gates are enabled, and live mode remains disabled. PR #104 removed the obsolete Accounts v2 payout request, PR #105 supplied the approved UK country, and PR #106 corrected Accounts v2 indexed retrieval parameters. Stripe created and retrieved the sandbox connected account successfully, proving the configured key and retrieval path work. The next protected request then reached Account Links v2 but incorrectly requested `account_update` for the not-yet-onboarded account; Stripe accepted only `account_onboarding`. A focused server-state link selection correction is under review; no onboarding form, Connect payment or refund occurred. Evidence: `STRIPE_BILLING_SANDBOX_ACCEPTANCE_EVIDENCE_2026-07-25.md`.
