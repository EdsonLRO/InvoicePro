...
The free-generator conversion panel and optional one-time overview email are now a repository-only release candidate. Its migration is unapplied, its public Edge Function and website changes are undeployed, its feature gates remain disabled and no promotional email has been sent. Activation requires exact Owner approval for the migration, server secrets/configuration, function deployment, updated Privacy Notice publication and website release.
