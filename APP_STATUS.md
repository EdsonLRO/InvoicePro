...
- Release build `2026.07.28.1` is active at `https://app.tallyo.co.uk` behind Cloudflare Access. Registration and the signed-out footer link the approved Privacy Notice and Business-User Data Processing Terms; the terms are expressly incorporated into the account agreement. The approved recovery origin remains deployed in `mfa-recovery` with JWT verification retained; GitHub Pages and both local development origins remain allowlisted.
- The separate website is implemented and rebuilt in production mode at its Access-protected Cloudflare Pages address. The approved customer-facing Privacy Notice and Business-User Data Processing Terms are available at `/privacy/` and `/data-processing-terms/`; the Helper and browser-local document forms link the Privacy Notice. The website is not published on `tallyo.co.uk`.
...
- Complete the remaining operational checks and unrestricted production-release decision as separate Owner-gated stages. The focused legal/privacy publication is complete.

No current task authorises removing Access, switching the Supabase Site URL, publishing the website for unrestricted visitors, another live Stripe transaction or refund, existing live Owner-route redeployment, unrelated legal publication or public launch.
