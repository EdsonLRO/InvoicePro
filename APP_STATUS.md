...
PR #151 merged owner-managed complimentary access for an existing confirmed account selected by email, and app build `2026.09.09.1` is live. The Supabase migration ledger records that production change under `20260909122037`, while the repository file is `20260909115547`; this history-only mismatch must be reconciled before the next migration is applied. The design adds no voucher UI, Stripe object, subscription, payment or copied email field.

A focused Owner Console candidate is under review on branch `codex/owner-console`. It adds exact-email account lookup, grant/revoke controls for the existing complimentary-access mechanism, registered-email password reset, and an account-holder-confirmed MFA recovery path that forces new-authenticator enrolment. It exposes no business records, impersonation, bulk administration, password, reset link, factor identifier or provider secret. Migration `20260911170410`, protected Owner identity configuration, `owner-account-admin`, the updated `mfa-recovery` function and app build `2026.09.11.1` are not deployed.
