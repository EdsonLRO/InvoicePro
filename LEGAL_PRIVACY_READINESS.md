...
| Cloudflare Turnstile (selected; inactive) | Bot and automated Auth-abuse detection for sign-up, password sign-in, and reset requests | IP address, TLS/browser fingerprint signals, user agent, sitekey/origin and challenge data; no workspace/customer data intended | Cloudflare describes processor activity for site protection and controller activity for improving Turnstile. Review DPA, UK transfer terms, subprocessors, retention, provider terms, and final notice wording before activation. | Bounded frontend implementation approved; production activation blocked |
