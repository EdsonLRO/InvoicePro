...
- Supabase now has the approved live mode/approval settings, exact Price allowlist, app base URLs and five enabled Billing/Connect server gates. The public website/app commercial gates and the database-owner-only `subscription_write_enforcement` switch remain off. Empty unauthenticated requests to the five protected commercial actions and unsigned requests to both webhook functions returned HTTP 401 without creating a live provider object or transaction; focused Billing, Connect, entitlement and payment-integrity harnesses pass.
...
6. **Production release** - authoritative DNS, the Access-protected app hostname and live Billing/Connect provider configuration are prepared; separately approve controlled live acceptance, Access removal, Site URL cutover, website publication and final public release.
