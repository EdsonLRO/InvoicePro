...
- **MFA recovery backend is deployed but not fully accepted.** Existing primary/backup-factor recovery remains Verified and email-only bypass remains blocked. Schema, secret, function, boundary, privilege, rolled-back AAL/RLS, authenticated lifecycle, and audit-minimisation probes pass; live throttling, notification delivery, final legal disposition, and real-device acceptance remain before frontend publication.
