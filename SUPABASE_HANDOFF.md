...
> Recovery destination correction (repository only, 2026-09-11): `mfa-recovery` and `owner-account-admin` now pin recovery links to `https://app.tallyo.co.uk`; deployment is pending. Do not change shared `APP_BASE_URL` to fix this issue, because unrelated payment/email functions also consume it. Existing mailed links are not rewritten.
