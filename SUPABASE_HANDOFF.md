...
- **Undeployed recovery candidate:** `supabase/functions/mfa-recovery/index.ts` plus migration `20260716133734_mfa_recovery_codes.sql` implement AAL2-only code generation, HMAC-SHA256-only database storage, five-attempt/15-minute locking, server-side factor removal, global session invalidation, a database-enforced recovery lock, and forced TOTP re-enrolment. Local frozen checks and focused harnesses pass. Production remains deny-by-default until the approval-gated backend-first deployment and acceptance in `MFA_RECOVERY_RUNBOOK.md`.
...
- **MFA recovery is implemented but not deployed.** Existing primary/backup-factor recovery remains Verified and email-only bypass remains blocked. The all-factors-lost candidate adds server-managed one-time codes and database-enforced forced re-enrolment; production keeps the approved deny-by-default response until its migration, secret, function, live probes, and browser acceptance pass.
