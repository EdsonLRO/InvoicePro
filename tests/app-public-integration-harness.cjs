...
const appBuildSource = fs.readFileSync(path.join(root, 'scripts', 'build-app-pages.mjs'), 'utf8');
...
assert.match(indexSource, /publicSiteLink\('cookies\/'\)/, 'registration and auth footer must link to the published Cookie Notice');
...
assert.match(workerSource, /tallyo-shell-2026-07-31-2/, 'service-worker cache marker must match this build');
