...
(async () => {
  ...
  assert.deepEqual(cacheNames, ['tallyo-shell-2026-07-31-1']);
  const shellStore = harness.stores.get(cacheNames[0]);
  for (const asset of ['./', './index.html', './tailwind.css', './analytics-consent.css', './analytics-consent.mjs', './analytics-app.js', './app-user-messages.js', './config.js', './app-help-install.js', './manifest.json', './tallyo-mark.png', './tallyo-wordmark-white.png', './icon-192.png', './icon-512.png']) {
  ...
  assert.match(indexSource, /const APP_BUILD = '2026\.07\.31\.1';/, 'frontend build marker must match the worker release');
