...
(async () => {
  ...
  assert.deepEqual(cacheNames, ['tallyo-shell-2026-09-20-4']);
  const shellStore = harness.stores.get(cacheNames[0]);
  for (const asset of ['./', './index.html', './tailwind.css', './analytics-consent.css', './analytics-consent.mjs', './analytics-app.js', './app-user-messages.js', './customer-csv-import.js', './config.js', './app-help-install.js', './manifest.json', './tallyo-mark.png', './tallyo-wordmark-white.png', './icon-192.png', './icon-512.png']) {
  ...
  assert.match(indexSource, /const APP_BUILD = '2026\.09\.20\.4';/, 'frontend build marker must match the worker release');
