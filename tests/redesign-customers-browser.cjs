...
(async () => {
  ...
  try {
    ...
    await page.goBack(); await page.waitForURL('**/#customers-detail');
    await detail.getByRole('heading', { name: 'Renamed fictional studio' }).waitFor();
    assert.equal(await detail.locator('h2').evaluate(el => el === document.activeElement), true, 'Back restores the exact customer detail');
    await page.goto(origin + '/#customers'); await page.locator('#preview-warning').waitFor();
    await customers.locator('.customer-name').click();
    await detail.getByRole('button', { name: 'Open document editor', exact: true }).click();
    await page.waitForURL('**/#create');
    await page.goBack(); await page.waitForURL('**/#customers-detail');
    await detail.locator('h2').waitFor();
    ...
    await page.goBack(); await page.waitForURL('**/#customers-detail');
    await detail.locator('h2').waitFor();
    await page.goForward(); await page.waitForURL('**/#recurring-form');
    assert.equal(await recurring.getByLabel('Frequency', { exact: true }).inputValue(), 'monthly', 'Forward restores the same schedule');
    ...
    console.log('Step 5 browser passed: exact Back/Forward customer-document-schedule context, customer links/edit/snapshots, schedule edit/pause/resume, empty states, focus, 320–1440px layout, Account/Owner route integration; no external requests or errors.');
