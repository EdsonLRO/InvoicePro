...
assert.match(edge, /admin\.auth\.admin\.mfa\.deleteFactor\(\{\s*userId,\s*id: factor\.id,?\s*\}\)/);
...
assert.match(html, /Lost access to every authenticator and your recovery codes\?/);
assert.match(html, /invokeMfaRecovery\('request-owner'\)/);
assert.match(html, /invokeMfaRecovery\('confirm-owner', \{ token \}\)/);
assert.match(edge, /action === "request-owner"[\s\S]*aal\.currentLevel !== "aal1"[\s\S]*aal\.nextLevel !== "aal2"/);
assert.match(edge, /action === "confirm-owner"[\s\S]*confirm_owner_mfa_recovery_request/);
assert.match(edge, /crypto\.randomUUID\(\)[\s\S]*hmacCode\(pepper, token\)/);
assert.doesNotMatch(edge, /console\.(?:log|error|warn)\([^\n]*(?:confirmationUrl|tokenHash|body\.token)/i);
