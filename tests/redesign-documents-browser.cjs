...
(async () => {
  ...
  try {
    ...
    for (const label of ['Duplicate', 'Email', 'PDF', 'Cancel selected', 'Delete']) assert.ok(await docs.getByRole('button', { name: label, exact: true }).isVisible());
    await docs.getByRole('button', { name: 'Cancel selected', exact: true }).click();
    assert.ok(await page.getByText('Selection can’t be cancelled', { exact: true }).isVisible(), 'mixed paid selection is rejected before any cancellation');
    assert.equal(await page.getByRole('dialog', { name: /Cancel .* selected documents/ }).count(), 0);
    ...
    if (await target.locator('details').getAttribute('open') === null) await target.locator('summary').click();
    await target.getByRole('button', { name: 'Cancel invoice', exact: true }).click();
    const singleCancel = page.getByRole('dialog', { name: 'Cancel invoice INV-1042?' });
    assert.ok(await singleCancel.isVisible());
    assert.match(await singleCancel.innerText(), /does not delete the invoice or quote/i);
    await singleCancel.getByRole('button', { name: 'Cancel invoice', exact: true }).click();
    await page.getByText('Invoice cancelled', { exact: true }).waitFor();
    assert.match(await target.locator('.record-status').innerText(), /Cancelled/);
    assert.equal(await target.getByRole('button', { name: 'Cancel invoice', exact: true }).count(), 0, 'already-cancelled record no longer offers cancellation');
    await page.getByRole('button', { name: 'Dismiss notification' }).click();

    const quoteRecord = records.filter({ has: page.getByRole('button', { name: 'Open Quote QUO-0217', exact: true }) });
    const currentRecord = records.filter({ has: page.getByRole('button', { name: 'Open Invoice INV-1046', exact: true }) });
    await quoteRecord.getByRole('checkbox').check();
    await currentRecord.getByRole('checkbox').check();
    await docs.getByRole('button', { name: 'Cancel selected', exact: true }).click();
    const bulkCancel = page.getByRole('dialog', { name: 'Cancel 2 selected documents?' });
    assert.ok(await bulkCancel.isVisible());
    await bulkCancel.getByRole('button', { name: 'Cancel 2 documents', exact: true }).click();
    await page.getByText('2 documents cancelled', { exact: true }).waitFor();
    assert.match(await quoteRecord.locator('.record-status').innerText(), /Cancelled/);
    assert.match(await currentRecord.locator('.record-status').innerText(), /Cancelled/);
    assert.equal(await docs.locator('input:checked').count(), 0, 'bulk cancellation clears completed selections');
    await page.getByRole('button', { name: 'Dismiss notification' }).click();
    await target.locator('summary').click();
