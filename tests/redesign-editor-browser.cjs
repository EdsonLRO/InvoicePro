...
(async () => {
  ...
  try {
    ...
    const statusAction = details.getByRole('combobox', { name: 'Status action', exact: true });
    assert.ok(await statusAction.isVisible());
    assert.deepEqual(await statusAction.locator('option').allTextContents(), ['Choose an action…', 'Mark as sent', 'Cancel document']);
    assert.equal(await details.locator('.editor-status-readout .record-badge').innerText(), 'Draft');
    assert.equal(await vmRead("vm.effectiveStatus({docType:'quote',status:'Sent',totals:{grandTotal:100},payments:[],dueDate:'2020-01-01'})"), 'Sent');
    assert.equal(await vmRead("vm.effectiveStatus({docType:'credit',status:'Sent',totals:{grandTotal:100},payments:[{amount:100}],dueDate:'2020-01-01'})"), 'Sent');
    await statusAction.selectOption('Sent');
    assert.equal(await vmRead('vm.draft.status'), 'Sent');
    assert.equal(await details.locator('.editor-status-readout .record-badge').innerText(), 'Sent');
    assert.deepEqual(await statusAction.locator('option').allTextContents(), ['Choose an action…', 'Cancel document']);
    ...
    // Preview fits the phone viewport without changing the 800px desktop/PDF canvas.
    await page.setViewportSize({ width: 390, height: 1000 });
    const previewSurface = page.locator('.editor-preview-surface');
    const mobilePreviewGeometry = await Promise.all([previewSurface, page.locator('#invoice-canvas')].map(locator => locator.boundingBox()));
    assert.ok(mobilePreviewGeometry[1].width <= mobilePreviewGeometry[0].width + 1, 'mobile preview canvas fits its surface');
    assert.ok(mobilePreviewGeometry[1].x - mobilePreviewGeometry[0].x >= 3, 'mobile preview keeps its left rounded edge inset');
    assert.ok((mobilePreviewGeometry[0].x + mobilePreviewGeometry[0].width) - (mobilePreviewGeometry[1].x + mobilePreviewGeometry[1].width) >= 3, 'mobile preview keeps its right rounded edge inset');
    const totalColumnBox = await page.locator('.pdf-line-items th').filter({ hasText: 'Total' }).boundingBox();
    assert.ok(totalColumnBox.x + totalColumnBox.width <= mobilePreviewGeometry[0].x + mobilePreviewGeometry[0].width + 1, 'mobile preview keeps the complete line-item table visible');
    const previewEdges = await page.evaluate(() => {
      const canvas = getComputedStyle(document.querySelector('#invoice-canvas'));
      const total = getComputedStyle([...document.querySelectorAll('.pdf-line-items th')].find(cell => cell.textContent.trim() === 'Total'));
      return { canvasBorder: parseFloat(canvas.borderRightWidth), canvasRadius: parseFloat(canvas.borderTopRightRadius), totalRadius: parseFloat(total.borderTopRightRadius) };
    });
    assert.ok(previewEdges.canvasBorder >= 1 && previewEdges.canvasRadius >= 8, 'mobile preview shows the complete rounded white page edge');
    assert.ok(previewEdges.totalRadius >= 6, 'mobile preview rounds the visible right edge of the line-item heading');
    assert.equal(await previewSurface.evaluate(el => el.scrollWidth > el.clientWidth + 1), false, 'mobile preview has no horizontal scrollbar');
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false, 'mobile preview does not overflow the page');
    await mkdir(path.join(root, 'tmp/redesign-evidence'), { recursive: true });
    await page.screenshot({ path: path.join(root, 'tmp/redesign-evidence/step4-mobile-preview.png') });
    await page.setViewportSize({ width: 1440, height: 1000 });
    assert.equal(Math.round((await page.locator('#invoice-canvas').boundingBox()).width), 800, 'desktop preview keeps the printable width');
    assert.ok(await page.locator('.pdf-line-items th').filter({ hasText: 'Total' }).evaluate(el => parseFloat(getComputedStyle(el).borderTopRightRadius) >= 6), 'desktop preview also rounds the visible table heading edge');
