...
(async () => {
  ...
  try {
    ...
    const expandAll = page.locator('.editor-header').getByRole('button', { name: 'Expand all', exact: true });
    assert.ok(await expandAll.isVisible());
    await expandAll.click();
    assert.equal(await form.locator('details[open]').count(), 5, 'all five main sections expand for a new document');
    assert.equal(await page.locator('.editor-aux[open]').count(), 2, 'recurring and reminders are included in Expand all');
    await page.locator('.editor-header').getByRole('button', { name: 'Collapse all', exact: true }).click();
    assert.equal(await form.locator('details[open]').count(), 0);
    assert.equal(await page.locator('.editor-aux[open]').count(), 0);
    ...
    assert.match(await item.locator('.editor-line-total').innerText(), /£162\.00/);
    await items.getByRole('button', { name: 'Prices include tax' }).click();
    assert.equal(await vmRead('vm.totals.grandTotal'), 135);
    assert.match(await item.locator('.editor-line-total').innerText(), /£135\.00/);
    await items.getByRole('button', { name: 'Add tax on top' }).click();
    await items.getByLabel('Discount percentage').fill('10');
    await items.getByLabel('Shipping').fill('5');
    assert.equal(await vmRead('vm.totals.grandTotal'), 150.8);
    await vmRead("(Object.assign(vm.draft.items[0], {qty:1,price:501,discount:0,tax:0}), vm.draft.shippingCost=0, true)");
    await items.locator('.editor-discount-mode').getByRole('button', { name: '£', exact: true }).click();
    await items.getByLabel('Discount amount').fill('26');
    assert.equal(await vmRead('vm.totals.grandTotal'), 475, 'fixed discount reaches an exact target total');
    await items.locator('.editor-discount-mode').getByRole('button', { name: '%', exact: true }).click();
    assert.ok(Math.abs((await vmRead('vm.draft.globalDiscount')) - (26 / 501 * 100)) < 0.000001, 'fixed amount converts precisely for storage');
    await vmRead("(Object.assign(vm.draft.items[0], {qty:1.5,price:100,discount:10,tax:20}), vm.draft.globalDiscount=10, vm.draft.shippingCost=5, true)");
    ...
    const secondItem = items.locator('.editor-item').nth(1);
    await secondItem.getByLabel('Add from Products & services').selectOption('__new__');
    const quickItem = page.getByRole('dialog', { name: 'Add product or service' });
    await quickItem.getByLabel('Name *').fill('Fictional workshop');
    await quickItem.getByLabel('Description (optional)').fill('Demonstration only');
    await quickItem.getByLabel('Default price').fill('75');
    await quickItem.getByRole('button', { name: 'Save & select' }).click();
    assert.equal(await secondItem.getByLabel('Description').inputValue(), 'Fictional workshop');
    assert.equal(await secondItem.getByLabel('Unit price', { exact: false }).inputValue(), '75');
    ...
    await vmRead("vm.company.brandColor = '#dbe4f5'");
    assert.equal(await vmRead('vm.brandInk'), '#1e293b');
    const totalInk = await page.locator('#invoice-canvas .pdf-summary-block .text-xl.font-bold span').last().evaluate(el => getComputedStyle(el).color);
    assert.equal(totalInk, 'rgb(30, 41, 59)', 'light branding uses a contrast-safe total amount');
    ...
    await vmRead("(vm.company.brandColor = '#dbe4f5', true)");
    ...
    const evidencePdf = path.join(root, 'tmp/redesign-evidence/invoice-editor-test.pdf');
    await copyFile(pdfPath, evidencePdf);
    const bytes = await readFile(evidencePdf);
