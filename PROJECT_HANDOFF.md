...
- **Supabase has no native recovery codes.** Tallyo supports a second authenticator and blocks email-only MFA recovery. The server-managed recovery backend is deployed with HMAC-only storage, throttling, factor/session cleanup, database-enforced recovery lock, and forced TOTP re-enrolment. Boundary, privilege, AAL, rolled-back RLS, authenticated lifecycle, and audit-minimisation probes pass; live throttling, notification delivery, final legal disposition, and real-device acceptance remain before publishing the frontend. Supabase leaked-password protection remains enabled and tested.
