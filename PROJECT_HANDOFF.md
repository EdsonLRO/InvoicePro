...
- **Supabase has no native recovery codes.** Tallyo supports a second authenticator and blocks email-only MFA recovery. The server-managed recovery backend is deployed with HMAC-only storage, throttling, factor/session cleanup, database-enforced recovery lock, and forced TOTP re-enrolment. Boundary, privilege, AAL, rolled-back RLS, authenticated lifecycle, audit-minimisation, and rollback-only live-throttling probes pass; notification delivery, final legal disposition, and real-device acceptance remain before publishing the frontend. Supabase leaked-password protection remains enabled and tested.
...
4. **Current hardening priorities:** complete notification delivery, final legal disposition, and real-device acceptance for the backend-deployed all-factors-lost MFA recovery candidate in `MFA_RECOVERY_RUNBOOK.md`; operationally test and legally review `PAYMENT_OPERATIONS_RUNBOOK.md`; complete the blocked legal/privacy work in `LEGAL_PRIVACY_READINESS.md`; and finish provider decisions in `DEFERRED_MANUAL_CONFIGURATION.md`. Recovery throttling has rollback-only production evidence. Controlled desktop/mobile account-export acceptance, corrected production audit metadata, and PWA update-across-deployment behaviour are Verified.
