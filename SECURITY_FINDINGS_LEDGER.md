...
- **Repository verification:** `tests/sensitive-reauth-captcha-harness.cjs` covers missing, cancelled, expired, provider-error, stale-callback, one-use forwarding, rollback, CAPTCHA rejection, throttling, wrong-password and network paths, plus retained password-change and global-sign-out/MFA/audit ordering. `tests/auth-captcha-harness.cjs`, `tests/session-expiry-harness.cjs`, `tests/security-workflow-harness.cjs`, `tests/scale-accessibility-safety-harness.cjs` and `tests/pwa-update-harness.cjs` pass. Inline script parsing is included in the focused harnesses. Diff/secret validation is recorded in the task.
...
- **Status after remediation:** Repository implementation verified; production publication and live acceptance remain pending exact Owner approval.
