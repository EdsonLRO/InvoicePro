...
| D10 | Vendor and transfer acceptance | Confirm applicable contracts and account tiers; complete role, subprocessor, location, transfer-mechanism, transfer-risk, deletion, support, and breach-notification review for Supabase, Resend, Stripe, GitHub Pages, pending Google Workspace, and dormant Turnstile. | Owner contract acceptance; qualified transfer review | Blocks provider production use where unresolved |
| D11 | Customer-facing commitments | Unpublished working drafts now cover a product-specific Privacy Notice, Terms, business-user data-processing terms, support/complaints wording, and payment/refund/dispute wording. Replace placeholders and approve final wording only after the underlying decisions and implementation are settled. | Qualified legal review; Product, Payments, Privacy and Release verification | Blocks legal publication and onboarding |
| D12 | ICO fee and accountability records | The internal fee worksheet, provisional DPIA-style assessment, LIAs and review calendar are prepared without private facts. Complete the official fee self-assessment using the real organisation facts and approve the final ROPA/assessment outcomes. | Owner facts; qualified advice if uncertain | Blocks launch sign-off |
...
The D07 system decision is recorded in `RESTRICTED_CASE_SYSTEM_OPTIONS_2026-07-17.md`. On 2026-07-17, the Owner selected Google Workspace Business Standard as the proportionate candidate for restricted case operations. Browser verification later showed that Standard is not verified active and Google presented a trial/checkout, so purchase and contract acceptance are pending and no configuration occurred. Standard does not include Vault, but Vault is not an automatic launch requirement; `GOOGLE_WORKSPACE_PRIVACY_CASE_PLAN_2026-07-17.md` defines the restricted-access, audit, deadline, secure-delivery, manual retention/deletion and synthetic-acceptance conditions that would apply if separately purchased. D08-D09 use the Owner as the initial operator, with ad-hoc qualified escalation when a complex legal or material technical trigger occurs. A second operator, paid Workspace continuity service and cyber-response retainer are optional contingencies. The public comparison research and unsent drafts in `PROFESSIONAL_BACKUP_CANDIDATES_AND_DRAFTS_2026-07-17.md` are retained for future use; no candidate has been selected or contacted.

For D11-D12, four customer-document drafts are now visibly marked `DRAFT - NOT APPROVED - DO NOT PUBLISH`, and `ICO_ACCOUNTABILITY_PREPARATION_2026-07-17.md` contains the private-fact-free fee worksheet, provisional DPIA-style assessment, three LIA worksheets and review calendar. Preparation is complete enough for qualified review, but no wording, fee result, assessment conclusion or publication is approved. `LEGAL_OWNER_ACTIONS_BEFORE_LAUNCH_2026-07-17.md` is the consolidated remaining-action authority.
...
4. approval or rejection of the Business Standard trial/subscription after contract, price and vendor/transfer review; configuration remains a later separate action;
...
Privacy Notice, Terms, refund/support wording, and customer-facing data-processing terms now exist only as internal placeholders. They must remain unpublished and visibly marked `DRAFT - NOT APPROVED - DO NOT PUBLISH` until D01-D06 and D10 are sufficiently decided, the wording is reconciled to actual implementation, qualified review is complete, and the Owner separately approves publication.
