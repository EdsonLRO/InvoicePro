...
PR #105 merged the UK-country correction and only `manage-stripe-connect` advanced to version 17. Stripe then returned HTTP 200 for account creation, proving the configured Connect key is valid and creating the first synthetic sandbox connected account. The subsequent retrieval returned `invalid_fields` because Accounts v2 requires indexed array query parameters (`include[0]`, `include[1]`, and so on) rather than the former `include[]` encoding. PR #106 corrected that query encoding, passed focused checks and was merged and deployed only to `manage-stripe-connect` under exact Owner approval.

The protected retry authenticated successfully, retrieved the connected account and reached Account Links v2. Stripe rejected the requested `account_update` link because the account had not completed onboarding and accepted only `account_onboarding`. The focused correction makes the server derive the provider flow from its saved provider state rather than the browser request: every non-active account uses onboarding to collect outstanding requirements, while update is reserved for an active account. The refresh URL carries that effective action so regeneration uses the same reviewed Account Link parameters. No onboarding form, connected payment or refund resulted from the rejected request.
