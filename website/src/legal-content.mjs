...
export const privacyNotice = `
  ...
  <article class="legal-document">
    ...
    <section>
      ...
      <h3>AI Helper</h3>
      <p>The public AI Helper answers questions about public Tallyo product information. To understand follow-up questions, Tallyo sends the visitor's current message, up to six recent exchanges from the same open page and the reviewed public Tallyo knowledge catalogue to OpenAI. The Helper is not designed to access account records or private business/customer data, and visitors must not enter that information.</p>
      <p>Tallyo keeps this recent conversation only in the open page and clears it when the visitor resets the Helper, reloads or leaves the page. Tallyo does not intentionally store it on its systems. OpenAI's standard API abuse-monitoring logs may retain prompt and response content for up to 30 days unless a separately approved reduced-retention control applies. Cloudflare uses a short-lived, protected network-derived key to enforce the Helper rate limit.</p>
    </section>
