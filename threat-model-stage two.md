...
| F-08 | No password policy / no MFA | Medium | A07 | **Partially remediated** | TOTP MFA, fail-closed assurance checks, local password rules, primary-specific and backup-specific masked recovery, backup-authenticator support, provider leaked-password rejection, and an interim deny-by-default lost-factor response are implemented and tested/approved. A robust paid/public-launch recovery method remains |
| F-09 | No data backup / durability | Medium | A04 (availability) | **Partially remediated** | Supabase Pro daily backups with seven-day retention and a recovery runbook are documented. Current backup evidence and a timed non-production restore test remain |
...
| F-11 | No audit logging (repudiation) | Low / Info | A09 | **Partially remediated** | Append-only `audit_events` covers provider events and selected sensitive app actions. Automation failures, backup/restore evidence, alerting and retention remain incomplete |
