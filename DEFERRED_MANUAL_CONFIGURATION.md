...
| CAPTCHA | Frontend prepared; not configured or enforced | The Owner approved the bounded Cloudflare Turnstile implementation. Dormant frontend support, exact-origin CSP, and automated tests are present; `TURNSTILE_SITE_KEY` remains blank and Supabase CAPTCHA remains off. Next: Owner provisions separate test/production Managed widgets with exact hostname, no pre-clearance, and masked secrets; browser acceptance follows. A separate approval is still required immediately before production enforcement. Never enable only the dashboard half. |
