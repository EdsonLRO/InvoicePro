...
- Desktop account-export acceptance passed on 2026-07-16 using only a dedicated test account: warning and download succeeded; JSON was readable; dataset counts matched the database; 25 tenant-owned rows had zero ownership mismatches; forbidden Auth/token fields were absent; and one production event was recorded. After PR #37 merged, `log-app-event` version 7 was deployed and a fresh event contained only `export_version` and `format`, with no `user_agent` or provider fields. Desktop acceptance and `SEC-LOG-006` are Verified. Repeat responsive/mobile acceptance; record only counts and pass/fail, never exported content.
