...
| CAPTCHA | Frontend prepared; not configured or enforced | The Owner approved the bounded Cloudflare Turnstile implementation. Dormant frontend support, exact-origin CSP, automated tests, and an internal vendor/transfer review are present; `TURNSTILE_SITE_KEY` remains blank and Supabase CAPTCHA remains off. Next: Owner accepts the provider terms and provisions a Managed test widget for `edsonlro.github.io` with no pre-clearance; controlled browser acceptance follows. The eventual production widget must be separate and use Tallyo's final app hostname. Secret entry and production enforcement remain later Owner-controlled steps. Never enable only the dashboard half. |
