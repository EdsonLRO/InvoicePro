...
| CAPTCHA | Test widget provisioned; app still not configured or enforced | The Owner approved the bounded Cloudflare Turnstile implementation, accepted the applicable account terms, and reported creating the Managed test widget for `edsonlro.github.io` with no pre-clearance on 2026-07-16. Dormant frontend support, exact-origin CSP, automated tests, and an internal vendor/transfer review are present; `TURNSTILE_SITE_KEY` remains blank and Supabase CAPTCHA remains off. Next: controlled desktop/mobile browser acceptance. The eventual production widget must be separate and use Tallyo's final app hostname. Secret entry and production enforcement remain later Owner-controlled steps. Never enable only the dashboard half. |
