...
**Recovery hardening added later.** The sign-in gate now fails closed: if Supabase cannot prove the required **Authenticator Assurance Level (AAL)** or cannot safely load the verified factors, Tallyo signs out the incomplete session instead of loading the app. A user can also add a second authenticator as a backup. Email access is not treated as a magic MFA bypass.

Because Supabase does not provide native recovery codes, Tallyo now has a backend-deployed server-managed candidate for the all-factors-lost case. An AAL2 user generates ten codes that are shown once. Only **HMAC-SHA256** values are stored, protected by a server-only pepper. Recovery needs the password session plus one code, invalidates the complete set, removes old factors and sessions, and locks business data in RLS until a new authenticator is verified. The backend has passed boundary, privilege, AAL, rolled-back RLS, authenticated lifecycle, and audit-minimisation probes; live throttling, notification delivery, final legal disposition, and real-device acceptance remain before the frontend is published. This does not make Tallyo NIST-certified or legally compliant.
...
**The same discipline now covers server code.** Every Supabase Edge Function pins `@supabase/supabase-js` to exact version `2.110.1`, and unused floating import aliases were removed. An import such as `@2` could silently select a newer package during a future deployment; the exact pin makes that dependency change visible and reviewable. Each function also has a frozen lockfile that records the expected remote-module digest. A read-only GitHub Actions gate checks every function and the focused security harnesses using immutable action commit SHAs, without receiving application secrets or repository write permission. Exact pins and locks reduce supply-chain drift but do not prove a package can never contain a vulnerability, so intentional updates still need security review and testing.
...
- preserve the Verified MFA controls and complete production deployment and acceptance of the all-factors-lost recovery candidate before paid/public onboarding;
...
- **MFA recovery is not fully live yet.** Tallyo supports a second authenticator and refuses email-only MFA bypass. The server-managed backend is deployed; authenticated generation/replacement, one-time recovery, forced re-enrolment, and audit-minimisation passed with privacy-safe evidence. Production still uses the interim deny-by-default user response until live throttling, notification delivery, final legal disposition, and real-device acceptance pass. Supabase leaked-password protection remains enabled, advisor-verified, and safely tested.
...
- **Deploy and accept the implemented all-factors-lost MFA recovery method** before paid/public onboarding. Keep the current server-side password minimum and breached-password screening active.
