...
Because Supabase does not provide native recovery codes, Tallyo now has a locally verified server-managed candidate for the all-factors-lost case. An AAL2 user generates ten codes that are shown once. Only **HMAC-SHA256** values are stored, protected by a server-only pepper. Recovery needs the password session plus one code, invalidates the complete set, removes old factors and sessions, and locks business data in RLS until a new authenticator is verified. The design includes attempt throttling and security notices. It is not deployed yet, and it does not make Tallyo NIST-certified or legally compliant.
...
- **MFA recovery is not live yet.** Tallyo supports a second authenticator and refuses email-only MFA bypass. The server-managed all-factors-lost candidate is implemented and locally security-tested, but production still uses the interim deny-by-default response until deployment and live acceptance pass. Supabase leaked-password protection remains enabled, advisor-verified, and safely tested.
