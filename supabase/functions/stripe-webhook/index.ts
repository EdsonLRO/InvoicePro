...
async function findPaymentAuditByIntent(admin: any, paymentIntentId: string) {
  ...
  const { data, error } = await admin.from("audit_events")
    ...
    .maybeSingle();
  if (error) {
    console.warn("payment intent audit lookup failed", error.message);
    return null;
  }
  return data;
}
...
async function handleCheckoutPaymentFailed(admin: any, event: any) {
  ...
  history.push({
    ts: nowISO,
    type: "payment_failed",
    text: `Stripe payment failed${amount > 0 ? ` for ${formatMoney(currency, amount)}` : ""}`,
...
async function handleRefund(admin: any, event: any) {
  ...
  if (!paymentIntentId || !refundId) return { ignored: "refund missing payment intent" };

  const paymentAudit = await findPaymentAuditByIntent(admin, paymentIntentId);
  if (!paymentAudit?.object_id || !paymentAudit?.user_id) return { ignored: "payment intent not known to Tallyo" };
  ...
  const currency = String(refund.currency || inv.currency || "GBP").toUpperCase();
  if (currency !== String(inv.currency || "GBP").toUpperCase()) return { ignored: "currency mismatch" };
  ...
  if (refundSucceeded && !alreadyRecorded) {
    ...
    history.push({
      ts: nowISO,
      type: "refund",
      text: `Stripe refund ${String(refund.status || "updated")} for ${formatMoney(currency, refundAmount)}`,
...
async function handleDispute(admin: any, event: any) {
  ...
  const currency = String(dispute.currency || inv.currency || "GBP").toUpperCase();
  ...
  history.push({
    ts: nowISO,
    type: "dispute",
    text: `${label}${amount > 0 ? ` for ${formatMoney(currency, amount)}` : ""}`,
