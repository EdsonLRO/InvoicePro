...
| Append-only audit events | In Progress | Provider events and selected app actions covered. Failed-email audits were minimised on 2026-07-14 so they no longer retain recipient addresses or raw provider bodies; backup/restore evidence remains. |
...
| Backup and restore | In Progress | Supabase Pro daily physical backups were listed as completed through 2026-07-14 and seven-day operational retention is documented; an Owner-approved timed non-production restore test remains. |
| Privacy/legal groundwork | In Progress | Internal data-flow, role, vendor, retention, rights, incident, claims and launch-block registers now exist in `LEGAL_PRIVACY_READINESS.md`; public notices, operational testing, final decisions and external review remain. |
...
5. Turn `LEGAL_PRIVACY_READINESS.md` from an internal blocked register into tested operations and approved customer-facing documents.
