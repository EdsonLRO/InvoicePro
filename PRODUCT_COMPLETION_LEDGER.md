...
| Account data export | Implemented | The Account page prepares a structured JSON export in the browser using the authenticated public Supabase client and existing RLS. It includes all six tenant datasets with stable pagination, limits Auth metadata, aborts on query/session failure, and records a minimal allowlisted success event. Focused success/failure harnesses pass; dedicated test-account desktop/mobile acceptance and production audit-event readback remain. |
...
| LEGAL-OPS-002 | Implement RLS-scoped account data export | High | Implemented | Security Agent with Legal Agent review | Sol / Terra | Source and focused harness complete; acceptance lock retained | `LEGAL_ACCOUNT_DATA_EXPORT_REVIEW_2026-07-15.md` records the approved-with-conditions scope. Test-account desktop/mobile acceptance and deployed `account_data_exported` evidence remain; third-party request routing, correction, deletion, secure case handling, and legal publication are separate blockers. |
...
4. Complete test-account desktop/mobile acceptance and deployed audit readback for the implemented account-data export, then resolve the separate correction, deletion, restricted-case, provider-assistance, and secure-delivery gaps from `LEGAL_TABLETOP_EVIDENCE_2026-07-15.md`.
