...
| Append-only audit events | In Progress | Provider events and selected app actions covered. Failed-email and overdue-reminder failure evidence is minimised. Deployed `log-app-event` v6 also covers manual payment recording/removal, manual status changes, and account-export success without customer, filename, count, exported-content, or free-text payment data. Privacy-safe restore evidence is recorded separately. |
| Account security | In Progress | Supabase Auth, email confirmation, leaked-password protection, a verified 12-character provider minimum, purpose-scoped custom Auth SMTP, fail-closed TOTP MFA, backup-authenticator support, MFA-gated masked password recovery, password change AAL2 handling, and local/all-devices logout. Primary/backup sign-in, protected factor removal, recovery rejection/acceptance, fail-closed lookup simulation, and dedicated-test-account Auth SMTP delivery passed. Unexpected provider sign-out clears in-memory business data while intentional logout stays quiet; merged deployment acceptance passed on 2026-07-15. The Owner-approved 7-day timebox and 24-hour inactivity timeout are active. Custom SMTP is configured at an initial 30 Auth emails per hour. AUTH-001 is Verified. Robust all-factors-lost recovery remains paid/public-launch work. |
| Account data export | Implemented | The Account page prepares a structured JSON export in the browser using the authenticated public Supabase client and existing RLS. It includes all six tenant datasets with stable pagination, limits Auth metadata, aborts on query/session failure, and records a minimal allowlisted success event. Focused success/failure harnesses pass; PR #29, GitHub Pages publication, deployed `log-app-event` v6 source readback, and clean immediate logs are confirmed. Dedicated test-account desktop/mobile acceptance and one successful production event row remain. |
...
| LEGAL-OPS-002 | Implement RLS-scoped account data export | High | Implemented | Security Agent with Legal Agent review | Sol / Terra | PR #29 merged and deployed; acceptance lock retained | `LEGAL_ACCOUNT_DATA_EXPORT_REVIEW_2026-07-15.md` records the approved-with-conditions scope. GitHub Pages and the v6 allowlist are read back; test-account desktop/mobile acceptance and one successful `account_data_exported` production row remain. Third-party request routing, correction, deletion, secure case handling, and legal publication are separate blockers. |
...
1. Resolve the remaining CAPTCHA/abuse-control decision in `DEFERRED_MANUAL_CONFIGURATION.md`. Custom SMTP delivery is Verified with a send-only domain-restricted credential and an initial 30-emails-per-hour limit. AUTH-001 is Verified; robust all-factors-lost recovery remains a paid/public-launch condition.
...
4. Complete test-account desktop/mobile acceptance and confirm one successful production audit row for the deployed account-data export, then resolve the separate correction, deletion, restricted-case, provider-assistance, and secure-delivery gaps from `LEGAL_TABLETOP_EVIDENCE_2026-07-15.md`.
5. Obtain approval for customer-facing legal documents and any required external professional review before public onboarding.
