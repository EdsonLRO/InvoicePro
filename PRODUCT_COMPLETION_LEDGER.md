...
| SUPPLY-001 | Enforce Edge Function and security-harness checks in CI | Medium | Implemented | Security Agent | Terra with Sol review | Focused branch active; remote CI pending | Immutable action pins, exact Deno `2.2.15` LTS, frozen v4 per-function locks, all nine checks, workflow self-audit, and focused Node harnesses pass locally without secrets or write permissions. Verify the first GitHub run before release. |
