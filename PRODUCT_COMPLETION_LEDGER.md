...
| SUPPLY-002 | Require the verified security gate on `main` | Medium | Implemented | Security Agent | Sol review | Focused PR enforcement proof pending | Owner-approved GitHub ruleset `18994100` is active for default `main`, has no bypass actors, blocks deletion/force-push, requires pull requests with zero approvals, and requires the GitHub Actions `verify` check on the latest branch state. |
