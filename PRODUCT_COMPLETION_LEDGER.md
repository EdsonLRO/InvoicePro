...
| Final mobile/PDF regression | In Progress | Public shell passed desktop and 390x844 layout checks. An authenticated 24-row, three-page desktop PDF passed row-boundary and continuation checks on 2026-07-15. The optimized exporter and authenticated mobile download still need post-deployment device acceptance. |
...
3. Deploy the PDF size hardening, then complete authenticated mobile PDF and PWA install/offline/update acceptance. The long desktop PDF and public shell checks are recorded.
