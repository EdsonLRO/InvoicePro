...
| Account security | In Progress | Supabase Auth, email confirmation, fail-closed TOTP MFA, backup-authenticator support, MFA-gated masked password recovery, password change AAL2 handling, and local/all-devices logout. Primary/backup sign-in, protected factor removal, and one enrolled-factor-gated password-recovery path passed browser acceptance on 2026-07-14; negative recovery tests, lookup-error simulation, and an all-factors-lost support process remain. |
...
3. Complete AUTH-001 acceptance for wrong-code and factor-specific password recovery, email-only bypass rejection, privacy-safe audit review, and simulated assurance/factor lookup failure; MFA sign-in, backup-factor lifecycle, and one successful MFA-gated recovery path passed on 2026-07-14.
