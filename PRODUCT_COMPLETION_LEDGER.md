...
| AUTO-001 | Authenticate privileged scheduled automation calls | High | Verified | Security Agent | Sol / Terra | Scheduled acceptance complete | Current `generate-recurring` retains unsigned-call rejection, both cron jobs use the Vault-backed automation secret, and the natural 06:00/09:00 UTC calls reached their protected functions with HTTP 200 on 2026-07-14. |
...
| AUTO-003 | Preserve scheduled automation response evidence | Medium | Implemented | Security Agent | Sol / Terra | Live cron commands hardened; scheduled acceptance unlocked | Both active jobs retain their original schedules, Vault-backed authentication, and now use explicit 30-second pg_net timeouts. Security advisors are clear and neither function was invoked during the change. Confirm the next natural scheduled responses before marking Verified. |
