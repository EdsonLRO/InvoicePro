...
| Account security | In Progress | Supabase Auth, email confirmation, fail-closed TOTP MFA, backup-authenticator support, MFA-gated masked password recovery, password change AAL2 handling, and local/all-devices logout. Primary/backup sign-in and protected factor removal passed browser acceptance on 2026-07-14; password recovery, lookup-error simulation, and an all-factors-lost support process remain. |
...
3. Complete AUTH-001 acceptance for masked MFA-gated password recovery, email-only bypass rejection, and simulated assurance/factor lookup failure; MFA sign-in and backup-factor lifecycle acceptance passed on 2026-07-14.
